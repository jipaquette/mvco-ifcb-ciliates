%seasonal groups cruise

%DO NOT KNOW HOW TO MAKE MEAN LINES WITH CRUISE DATA NOT BEING SMOOTHED
%CLIMATOLOGIES LIKE MVCO

%%
%Creating variables
cruise = load('/Volumes/IFCB_products/NESLTER_transect/summary/count_group_class_withTS.mat');
cruise2 = table2timetable(cruise.classcount_opt_adhoc_merge, 'RowTimes', cruise.meta_data.datetime);
cruise2.ml = cruise.meta_data.ml_analyzed;
cruise2.pid = [];
cruise2(cruise2.ml<0,:) = [];
mdate = datenum(cruise2.Time);

doy = day(cruise.meta_data.datetime, 'dayofyear');
cruise.meta_data.year = year(cruise.meta_data.datetime); 

%OLD MVCO SYNTAX
group_table = readtable('IFCB_classlist_type_ciliate_namechange.csv');
[~, ia, ib] = intersect(group_table.CNN_classlist, cruise.classcount_opt_adhoc_merge.Properties.VariableNames);
ciliate_ind = ib(find(group_table.Ciliate(ia)));
ciliate_label = cruise.ciliate_label;
...
for count_all = ciliate_to_plot_all
    tr_val = 4;
    var2plot = cruise2{:,(count_all)}./cruise2.ml;
    [ mdate_mat, y_mat, yearlist, yd ] = timeseries2ydmat( mdate, var2plot );
...
for count_sprfall = ciliate_to_plot_springfall
    var2plot = cruise2{:,(count_sprfall)}./cruise2.ml;
    [ mdate_mat, y_mat, yearlist, yd ] = timeseries2ydmat( mdate, var2plot );
    tr_val_sf = 4;
    [ xmean, xstd ] = smoothed_climatology( y_mat.^(1/tr_val_sf), 10);
    plot(yd, xmean.^tr_val_sf, '-', 'linewidth', 2)
...
for count_fall = ciliate_to_plot_fall
    var2plot = cruise2{:,(count_fall)}./cruise2.ml;
    [ mdate_mat, y_mat, yearlist, yd ] = timeseries2ydmat( mdate, var2plot );
    tr_val_f = 4;
    [ xmean, xstd ] = smoothed_climatology( y_mat.^(1/tr_val_f) , 10);
    plot(yd, xmean.^tr_val_f, '-', 'linewidth', 2)
